...
public class Mp3Player : MonoBehaviour 
{
	...
	RectTransform[] images = new RectTransform[64];
	float[] topTimer = new float[64];
	public GameObject prefab;
	private Vector2 gravity = new Vector3(0.0f, -9.8f);  
	float val = 200;
	...
	void Start()
	{
		...
		for (int i = 0; i < images.Length; i++) 
		{
			...
			images[i] = obj.GetComponent<RectTransform>();
			obj.transform.SetParent(prefab.transform.parent);
			obj.transform.localPosition = new Vector3 (startX + i * stepX, startY, 0);
			obj.SetActive (true);
			images[i].sizeDelta = new Vector2(8, 0);
		}
	}
	...
	void Update()
	{
		...
		for (int i = 0; i < images.Length; i ++)
		{
			float y = Mathf.Clamp (samples [i] * (val + i * i), 0, val);
			if (y > images [i].sizeDelta.y) 
			{
				images [i].sizeDelta = new Vector2 (8, y);
				topTimer[i] = Time.time;
			} 
			else 
			{
				images [i].sizeDelta += gravity * (Time.time - topTimer[i]);
			}
		}
	}
}
